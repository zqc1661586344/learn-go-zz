...
1. GC的流程（并不是完全停止程序，而是通过三色标记法和混合写屏障将STW控制在极短时间内）
- 标记开始（STW，微妙级）：
  - 触发：当堆内存增长到阈值时（GOGC控制）触发GC（栈内存由goroutine退出时自动释放，不参与GC）
  - STW：暂停所有的goroutine
  - 从根对象标记可达对象
- 并发标记（无STW）：
  - 从灰色队列中取出对象，标记为黑色
...
2. GC的本质是使用三色标记法和混合写屏障，避免长时间STW 
- 白色：初始状态，未被标记，可能是垃圾
- 灰色：中间状态，已被标记，但是其引用未被完全检查，需要继续扫描
- 黑色：稳定状态，已被标记，且所有的引用都已检查
  
3. 相关问题
- GC的使用方式
  - 自动触发：堆内存增长到达一个阈值（上次 GC 后堆大小 *(1+GOGC/100)）；定时触发；内存分配速度过快
  - 手动触发：通过通过runtime.GC()强制触发一次完整GC，会阻塞调用者直到GC完成
...
- 内存泄漏检测方式？
  ...
  - GODEBUG=gctrace=1观察堆内存是否持续增长

- 如何优化GC的性能？（减少STW/降低GC开销）
  - 减少内存分配：比如服用对象池，避免频繁创建临时对象
  - 合理设置GOGC：IO密集型调大，内存敏感型调小
  - 减少大对象分配：
  - 避免长时间CPU占用
  - 监控GC指标
...
1. 底层数据结构
   ...
   type hchan struct {
      ...
      buf      unsafe.Pointer // 指向环形队列，缓冲区（存储缓冲channel数据）

      elemsize uint16         // 元素大小
      closed   uint32         // 关闭状态
      elemtype *_type         // 元素类型
      
      sendx    uint           // 发送索引（标记环形队列的读写位置）
      recvx    uint           // 接收索引（标记环形队列的读写位置）
      
      recvq    waitq          // 接收等待队列（存储阻塞的goroutine）
      sendq    waitq          // 发送等待队列（存储阻塞的goroutine）
      
      lock     mutex          // 互斥锁（保证多goroutine并发安全）
   }
   ```
2. 操作流程
- 发送流程
  - 加锁
  - 是否有等待的接收者，如果有则直接将数据拷贝到接收者
  - 如果没有接收者，且缓冲区不为空，将数据加入缓冲区
  - 否则就阻塞，加入到发送的等待队列中
  - 解锁
- 接收流程
  - 加锁
  - 是否有等待的发送者，如果有则直接从发送者获取数据
  - 如果没有等待的发送者，且缓冲区不为空，则从缓冲区拿数据
  - 否则就阻塞，加入到接收的等待队列
  - 解锁
3. 无缓冲Channel和有缓冲Channel
- 无缓冲Channel
  - `make(chan int)`
  -  同步通信；发送会阻塞，直到有goroutine接收；接收会阻塞，直到有goroutine发送
- 有缓冲Channel
  - `make(chan int, 2)` 
  - 异步通信；当缓冲区满时，发送操作会阻塞；当缓冲区为空时，接收操作会阻塞
4. Channel的基本操作
- 发送
- 接收
- 关闭
  ...
  - 对已关闭的Channel接收会返回零值
  - 重复关闭会panic
- 对nil的Channel发送、接收会永久阻塞
- 关闭nil的Channel会panic
5. 相关问题
- Channel数据存储在哪里？
  - 有缓冲的Channel，数据存在hchan结构体的buf指向的内存中
  - 无缓冲Channel，数据直接在发送和接收的goroutine之间传递
- Channel的缓冲区如何实现？
  - 是一个环形队列，通过sendx和recvx指针控制
- 为什么Channel的recvq和sendq是双向链表？
  -  方便在阻塞时添加和移除goroutine，实现FIFO
  -  